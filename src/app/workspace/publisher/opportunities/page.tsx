...
export default async function PublisherOpportunities({
  searchParams,
}: {
  ...
  const { data: opps } =
    oppIds.length > 0
      ? await supabase
          ...
          .in("id", oppIds)
      : { data: [] as { id: string; public_transaction_id: string; ping_attributes: Record<string, unknown>; vertical_id: string | null }[] };
  ...
  return (
    <WorkspaceShell
      ...
      title="Opportunities"
      subtitle="Sold and billable opportunities from your sources."
    >
      <div className="dashStats">
        ...
        <article>
          ...
          <strong>
            {money(
              (txns ?? []).length
                ? Math.round(earnings / (txns ?? []).length)
                : 0,
            )}
          </strong>
      ...
      <div className="dashPanel">
        ...
        {(txns ?? []).map((t) => {
          ...
          return (
            <div className="tableRow opp" key={t.id}>
              ...
              <span>{money(t.publisher_amount_cents)}</span>
        ...
        {!txns?.length && (
          <div className="tableRow">
            <span className="status">No sold opportunities yet.</span>
          </div>
        )}
      </div>
    </WorkspaceShell>
